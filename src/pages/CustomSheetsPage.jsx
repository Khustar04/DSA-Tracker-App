...
import { Trash, Plus, Pencil, Copy } from 'lucide-react';
...
export default function CustomSheetsPage() {
  ...
  const [publicSheets, setPublicSheets] = useState([]);
  ...
  const handleCreate = async (e) => {
    ...
    const newSheet = await supabaseService.createCustomSheet(user.id, newTitle, isPublic);
    if (newSheet) {
  ...
  const saveEdit = async (id) => {
    ...
    const prev = sheets.find((s) => s.id === id);
    await supabaseService.updateCustomSheet(user.id, id, {
      title: editTitle,
      is_public: prev?.is_public || false,
    });
  ...
  const handleVisibilityToggle = async (sheet) => {
    const nextValue = !sheet.is_public;
    await supabaseService.updateCustomSheet(user.id, sheet.id, {
      title: sheet.title,
      is_public: nextValue,
    });
    setSheets(sheets.map((s) => (s.id === sheet.id ? { ...s, is_public: nextValue } : s)));
    toast.success(nextValue ? 'Sheet is now public' : 'Sheet is now private');
  };

  const handleClone = async (sheetId) => {
    const cloned = await supabaseService.cloneCustomSheet(user.id, sheetId);
    if (cloned) {
      toast.success('Sheet cloned to your list');
      loadSheets();
    } else {
      ...
      <div className="bg-white/5 border border-white/10 p-6 rounded-xl">
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-mono rounded-lg border ${activeTab === 'mine' ? 'text-neon-green bg-neon-green/10 border-neon-green/20' : 'text-white/50 border-white/10'}`}
          >
            My Sheets
          </button>
          <button
            onClick={() => setActiveTab('public')}
            className={`px-3 py-1.5 text-xs font-mono rounded-lg border ${activeTab === 'public' ? 'text-neon-green bg-neon-green/10 border-neon-green/20' : 'text-white/50 border-white/10'}`}
          >
            Browse Public
          </button>
        </div>
        ...
        <label className="mt-3 inline-flex items-center gap-2 text-xs text-white/60">
          ...
          />
          Make this sheet public (others can browse and clone)
        </label>
      </div>

      {activeTab === 'mine' && sheets.length === 0 ? (
        <div className="text-center py-12 bg-white/5 border border-white/10 rounded-xl">
          <p className="text-white/40">You haven't created any custom sheets yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(activeTab === 'mine' ? sheets : publicSheets).map((sheet) => (
            <div key={sheet.id} className="bg-white/5 border border-white/10 rounded-xl p-4 flex flex-col justify-between hover:border-white/20 transition-colors group">
              <div className="flex items-start justify-between mb-4">
                ...
                {activeTab === 'mine' ? (
                  ...
                  <button onClick={() => handleClone(sheet.id)} className="text-white/40 hover:text-neon-green p-1" title="Clone sheet">
                    <Copy size={16} />
                  </button>
                )}
              </div>
              <div className="flex items-center justify-between mt-2">
                ...
                {activeTab === 'mine' ? (
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleVisibilityToggle(sheet)}
                      className={`text-[10px] font-mono px-2 py-1 rounded border ${sheet.is_public ? 'text-neon-green border-neon-green/30 bg-neon-green/10' : 'text-white/50 border-white/10'}`}
                    >
                  ...
                  <span className="text-[10px] font-mono text-white/40">By another user</span>
                )}
              </div>
            </div>
          ))}
