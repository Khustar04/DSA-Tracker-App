              ...
              <thead>
                <tr className="text-white/40 font-mono border-b border-white/5 bg-black/20">
                  ...
                  <th className="py-3 px-4 font-normal">Topics</th>
                    <th className="py-3 px-4 font-normal w-20 text-center">Links</th>
                ...
                {problems.map((problem, idx) => (
                  <tr key={problem.id} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center">
                      {problem.problem_link || problem.leetcode_link || problem.gfg_link ? (
                        <div className="flex items-center justify-center gap-1">
                          {problem.problem_link && (
                            <a href={problem.problem_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 text-white/70 transition-colors" title="Open link">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>
                            </a>
                          )}
                          {problem.leetcode_link && (
                            <a href={problem.leetcode_link} target="_blank" rel="noopener noreferrer" className="text-[9px] font-mono px-1.5 py-1 rounded border border-white/10 text-white/70 hover:text-neon-green">
                              LC
                            </a>
                          )}
                          {problem.gfg_link && (
                            <a href={problem.gfg_link} target="_blank" rel="noopener noreferrer" className="text-[9px] font-mono px-1.5 py-1 rounded border border-white/10 text-white/70 hover:text-neon-green">
                              GFG
                            </a>
                          )}
                        </div>
